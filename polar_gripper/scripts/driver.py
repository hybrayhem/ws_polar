...
class MoveGroupPythonInteface(object):
    def __init__(self):
        ...
        rospy.init_node('move_group_python_gripper', anonymous=True)
...
class GripperController:
    def __init__(self):
        rospy.Subscriber("/polar_gripper/command", String, self.callback)
    ...
    def callback(self, data):
        ...
        if data.data == 'close':
            self.move_group.go_to_joint_state(80)
        if data.data == 'open':
            self.move_group.go_to_joint_state(0)
        if data.data == 'semi_close':
            self.move_group.go_to_joint_state(56)
        if data.data == 'semi_open':
            self.move_group.go_to_joint_state(45)
